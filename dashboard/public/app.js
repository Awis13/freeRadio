(function () {
  ...
  // monitorMusicGain/mmMasterGain are read by setPlayerMuted below and are read
  // back through FRMixer live getters rather than being copied.
  //
  // The analyzer graph the faders and the duck envelope drive has not been
  // extracted yet, so it is injected as narrow getters instead of being reached
  // for; ensureAnalyzer reuses the callback shape player.js already receives.
  FRMixer.init({
    log: log,
    showError: showError,
    authFetch: authFetch,
  ...
  function setPlayerMuted(muted) {
    ...
    if (getGainNode() && getAudioCtx()) {
      ...
      getGainNode().gain.setValueAtTime(muted ? 0 : FRMixer.getMonitorMusicGain() * FRMixer.getMasterGain(), getAudioCtx().currentTime);
    }
  }
