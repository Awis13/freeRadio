/**
 * utils.js — Pure utility functions for the STUDIO 23 / FreeRadio dashboard.
 *
 ...
 *
 * Dual-target UMD module: this file is loaded directly by the browser as a
 * plain <script src="/utils.js"> (it attaches its public API to
 * window.FRUtils) AND required by the vitest suite via module.exports (CJS).
...
(function (factory) {
   ...
   * @returns {number}
   */
  function computeMixDur(bpm, mixMode) {
   ...
   */
  function getBroadcastPhase(state) {
   ...
   */
  function deriveUiMode(visualMode) {
    if (visualMode === 'live') {
   ...
   * @returns {string}
   */
  function uniquePlatformName(base, existingNames) {
